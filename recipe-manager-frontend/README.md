...
* [`src/recipe_manager_frontend/`](src/recipe_manager_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/recipe_manager_frontend/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `recipe-manager-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/recipe_manager_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns recipe-manager-frontend.example
  (:require [recipe-manager-frontend.config :as config])
